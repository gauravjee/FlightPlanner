...
// ---------------------------------------------------------------------------
// 2026-09-11: every generator below can now either produce its own PDF (the
// existing one-report-per-button behaviour, unchanged for all four existing
// call sites) or append itself as a new page to a doc the caller already
// owns. That second mode is what the DGCA audit pack needs: an inspector
// asks for one document, not four downloads the browser may throttle
// anyway.
//
// The generators draw at absolute page coordinates rather than tracking a
// cursor across pages, which is what makes this cheap — after addPage() the
// same drawing code lands on the new page untouched. Orientation is
// per-page, so the landscape maintenance log coexists with the portrait
// reports in one file.
// ---------------------------------------------------------------------------

/** A fresh doc, or a new page on the caller's. */
function section(into: jsPDF | undefined, orientation: 'portrait' | 'landscape' = 'portrait'): jsPDF {
  if (!into) return new jsPDF({ orientation });
  into.addPage('a4', orientation);
  return into;
}

/** Save only when we created the doc — an appended section is the caller's to save. */
function finish(doc: jsPDF, owned: boolean, filename: string): jsPDF {
  if (owned) doc.save(filename);
  return doc;
}
 ...
 */
export function generateStudentLogbook(student: StudentRecord, flights: FlightRecord[], into?: jsPDF): jsPDF {
  const owned = !into;
  const doc = section(into);
  ...
  // Save the PDF
  return finish(doc, owned, `${student.name.replace(/\s+/g, '_')}_Logbook.pdf`);
}
...
export function generateDailyFlyingReport(report: {
  ...
  return finish(doc, owned, `Daily_Flying_Report_${report.reportDate}.pdf`);
}
...
export function generateBreathAnalysisReport(report: {
  ...
  return finish(doc, owned, `Breath_Analysis_Report_${report.period}_${safeLabel}.pdf`);
}
/**
 ...
 */
// '02 Sep 2026', not '2/9/2026'.
//
// The other two report generators use toLocaleDateString('en-IN'), which
// emits D/M/YYYY. That is fine on an internal ops sheet and wrong here: a
// maintenance log is read by a regulator, possibly years later, possibly
// beside US-format paperwork, and "2/9/2026" is genuinely ambiguous between
// 2 September and 9 February. A named month cannot be misread. Deliberately
// inconsistent with the sibling reports — correctness on a compliance
// document beats matching a house style.
export function formatLogDate(date: string | null | undefined): string {
  if (!date) return '—';
...
export function generateMaintenanceLogReport(report: {
  ...
  // FTO name on the top line and a size up from the period beneath it: the
  // operating organisation is part of the record's identity — the first
  // thing an auditor matches against their own paperwork — whereas the date
  // range is a filter on that record. Ranked accordingly.
  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text(`Period: ${formatLogDate(report.from)} to ${formatLogDate(report.to)}`, pageWidth - 14, 24, { align: 'right' });
  ...
  doc.text(
    `Generated by FlightPro Manager on ${formatLogDate(new Date().toISOString())}. This printout is a record of maintenance carried out; it is not itself a Certificate of Release to Service.`,
    14, pageHeight - 8,
  );

  return finish(doc, owned, `Maintenance_Log_${report.aircraftReg}_${report.from}_to_${report.to}.pdf`);
}

// ---------------------------------------------------------------------------
// DGCA AUDIT PACK (2026-09-11)
//
// One PDF bundling the reports an inspector asks for over a date range,
// instead of four separate downloads. Adds no report of its own: it draws a
// cover page and then calls the four generators above in append mode, so
// each section is byte-for-byte the document the FTO already reviews and
// exports on its own page.
//
// WHAT THE COVER PAGE IS FOR. A pack assembled from saved snapshots can
// have holes — a day nobody generated a Daily Flying Report for produces no
// page. Silently omitting it makes the pack look complete when it is not,
// which is the worst outcome on a document handed to a regulator. Every
// such gap is listed on the cover by date.
// ---------------------------------------------------------------------------

type DailyFlyingArg = Parameters<typeof generateDailyFlyingReport>[0];
type MaintenanceLogArg = Parameters<typeof generateMaintenanceLogReport>[0];

export function generateAuditPack(pack: {
  ftoName?: string;
  from: string;
  to: string;
  dailyReports: DailyFlyingArg[];
  /** Dates in range with no saved Daily Flying Report — printed as gaps. */
  missingDates: string[];
  ba: { periodLabel: string; tests: BATest[] } | null;
  maintenance: MaintenanceLogArg[];
}): void {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  ...
  doc.setFontSize(20);
  doc.setFont('helvetica', 'bold');
  doc.text('DGCA Audit Pack', 14, 18);
  doc.setFontSize(11);
  doc.setFont('helvetica', 'normal');
  if (pack.ftoName) doc.text(pack.ftoName, 14, 28);
  doc.text(`${formatLogDate(pack.from)} to ${formatLogDate(pack.to)}`, 14, 35);

  doc.setTextColor(0, 0, 0);
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text('Contents', 14, 55);

  const contents: string[][] = [
    ['Daily Flying Reports', `${pack.dailyReports.length} day(s)`],
    ['Breath Analysis Report', pack.ba ? `${pack.ba.tests.length} test(s)` : 'No tests in range'],
    ['Aircraft Maintenance Logs', `${pack.maintenance.length} aircraft`],
  ];
  ...
  let y = ((doc as JsPDFWithAutoTable).lastAutoTable?.finalY || 80) + 14;

  // Gaps — the reason this page exists. Listed by date, never summarised
  // away, so nobody hands over a pack believing a missing day is an
  // uneventful one.
  if (pack.missingDates.length > 0) {
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(180, 40, 40);
    doc.text('Days with no generated Daily Flying Report', 14, y);
    y += 7;
    doc.setFontSize(8.5);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
    const gapText = doc.splitTextToSize(pack.missingDates.map(formatLogDate).join(',  '), pageWidth - 28);
    doc.text(gapText, 14, y);
    y += gapText.length * 4.5 + 6;
    doc.setFontSize(8);
    doc.setTextColor(90, 90, 90);
    doc.text(
      doc.splitTextToSize(
        'These days are absent from this pack because no report was generated for them. Generate them on the Daily Flying Report page and rebuild this pack if they are required.',
        pageWidth - 28,
      ),
      14, y,
    );
  ...
  doc.text(
    doc.splitTextToSize(
      'Each section that follows is the same document produced by its own report page. The Aircraft Maintenance Log is a record of maintenance carried out; it is not itself a Certificate of Release to Service.',
      pageWidth - 28,
    ),
    14, y,
  );
  ...
  // Sections, in the order an inspector reads them.
  pack.dailyReports.forEach(r => generateDailyFlyingReport(r, doc));
  if (pack.ba && pack.ba.tests.length > 0) {
    generateBreathAnalysisReport({ period: 'Custom', periodLabel: pack.ba.periodLabel, tests: pack.ba.tests }, doc);
  }
